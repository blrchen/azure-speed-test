  ...
  var latency = {
    ...
    report: function () {
      ...
      utils.getRegions().forEach(function (value) {
        tmpRegions.push({
          geographyGrouping: value.geographyGrouping,
          geography: value.geography,
          name: value.name,
          location: value.location,
          average: latency.latest[value.storageAccountName]
        });
      });

      tmpRegions.forEach(function (value) {
        if (value.average > 0) {
          var tdGeography = $('<td>').text(value.geography);
          var tdRegion = $('<td>').text(value.name);
          var tdLocation = $('<td>').text(value.location);
          var tdLatency = $('<td>').text(parseInt(value.average).toFixed(0) + ' ms');
      ...
      tmpRegions.forEach(function (value) {
        if (closest < 3) {
          closest++;
          var text = value.name + ' ( ' + value.location + ' )';
          closestTable.append($('<tr>').append($('<td>').text(text)).append($('<td>').text(parseInt(value.average).toFixed(0) + ' ms')));
        }
      });
    ...
    tickloop: function () {
      ...
      function tick() {
        ...
        // fill new data
        utils.getRegions().forEach(function (value) {
          var storageAccountName = value.storageAccountName;
          if (latency.latest[storageAccountName] > 0) {
            data[value.id].push({ value: latency.latest[storageAccountName] });
          }
        });
