namespace AzureSpeed.Web.App.Common
{
    ...
    public class LocalDataStoreContext
    {
        ...
        private Dictionary<string, string> regionNames;

        public LocalDataStoreContext(string root)
        {
            this.root = root;
        }

        public IEnumerable<StorageAccount> Accounts
        {
            get
            {
                if (accounts == null)
                {
                    string filePath = Path.Combine(root, "App_Data", "settings.json");
        ...
        public Dictionary<string, string> RegionNames
        {
            get
            {
                if (regionNames == null)
